class ez5.Scancode extends CUI.DOMElement
    ...
    initOpts: ->
        ...
        @addOpts
            type:
                check: [ez5.Scancode.TYPE_BAR, ez5.Scancode.TYPE_QR]
                default: ez5.Scancode.TYPE_BAR
            scancode_type:
                check: String
                default: ez5.Scancode.DEFAULT_BARCODE_TYPE
            mode:
                check: String
            download:
                check: Boolean
    ...
    render: (data, { displayName = "", fieldName, objectType } = {}) ->        
        isQR = @_type == ez5.Scancode.TYPE_QR
        if isQR
        ...
        if not data or not (CUI.util.isString(data) or CUI.util.isNumber(data))
            if @_mode == "editor"
                ...
                return @
            CUI.dom.empty(@__ratio) # No data, other mode than editor, remove the code.
            return @
        ...
        if @_mode != "pdf" && @_download
            fileName = "
                ...
                #{fieldName}
                -
                #{@_type}#{if @_type == ez5.Scancode.TYPE_BAR then " #{@_scancode_type}" else ""}
            "
